 ...
 *
 * 기본 임계값 근거(잠정, 오너 검토 필요): 자체 결제 데이터가 아직 없는 신규 서비스라 정교한 통계
 * 기반 설정이 불가능하다. 국내 커머스 업계에서 흔히 쓰이는 "연간 상위 20%/5%/1%" 식 티어링 관행을
 * 참고해, 12개월 순구매액 30만원(가벼운 반복구매)/100만원(단골)/300만원(핵심 고객) 을 잠정 3개
 * 경계로 잡았다 — 정확한 값은 실제 구매 데이터 축적 후 재조정 대상이다.
 */
