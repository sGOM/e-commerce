    ...
    <div className="space-y-6">
      ...
      {order.subOrders.map((sub) => (
        <div key={sub.subOrderId} className="rounded-xl border bg-white p-5">
          ...
          {sub.deliverySlotId != null && (
            <p className="mb-2 rounded bg-sky-50 px-2 py-1 text-xs text-sky-700">
              🕐 배송 슬롯 예약됨
              ...
              <li key={idx} className="flex justify-between text-sm">
                <span>
                  ...
                  {item.appliedSalePrice != null && (
                    <span className="ml-1.5 inline-flex items-center gap-1 rounded bg-red-50 px-1.5 py-0.5 text-xs font-semibold text-red-500">
                      ⏰ 특가
                    </span>
                  )}
                </span>
          ...
          {sub.status === 'SHIPPED' && onConfirmDelivery && (
            <button
              ...
              className="mt-3 w-full rounded-lg border border-indigo-200 py-2 text-sm font-semibold text-indigo-600 hover:bg-indigo-50 disabled:opacity-50"
            >
      ...
      <div className="rounded-xl border bg-white p-5">
        ...
        {order.isGift ? (
          ...
          <p className="text-sm text-slate-400">배송지 정보가 없습니다.</p>
        )}
      </div>
