...
- AC8: 무료배송(배송비 모델이 있다는 전제 하에 배송비 0원 처리). ✅ 2026-09-25 구현 — 기본 배송비·슬롯 추가요금 모두 0원.
...
- ~~무료배송 실제 로직(배송비 모델 확정 후)~~ → 2026-09-25 배송비 모델 확정과 함께 구현됨
- 멤버십 전용 상품/특가
...
1. **정기결제 PG 확정**: 토스페이먼츠 빌링 API를 쓸지, 별도 PG를 검토할지 — 계약/정산 조건 확인
   필요.
2. ~~**배송비 모델 부재**~~ ✅ 2026-09-25 해소 — 판매자당 기본 배송비 3,000원, 멤버십 무료배송 면제.
